...
//--------------------------------------------------------------
// 点光源・スポットライト配列(b5)の宣言はLighting.hlsli側にある。
//
// 【重要】.hlsliを書き換えてもこのファイルの更新日時は変わらず、
// AssetManagerのキャッシュ判定（.hlsl自身の日時とcsoの日時を比べるだけ）を
// すり抜けて古いcsoが使われ続ける。スロット番号を動かしたのに絵だけ壊れる、
// というコンパイルエラーの出ない事故になるため、.hlsliを触ったときは
