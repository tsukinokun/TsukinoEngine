...
namespace Tsukino::Renderer {
    ...
    public:
        ...
        //------------------------------------------------------------
        //! @brief  シャドウパスでカスケードの範囲外を間引くかを設定する関数
        //! @param  [in] enabled true: 間引く（既定）, false: 全て描く
        //! @note   見た目を変えるための口ではなく、負荷を切り分けるための口。
        //!         同じビルドで「カスケード数ぶん全部描いた場合」と比較できるようにしてある。
        //!         見た目が変わるなら間引きすぎている
        //------------------------------------------------------------
        void SetShadowCullingEnabled(bool enabled);
