...
**1.x の間も変わりうる** — `Tsukino.Renderer`。`Renderer` はパスごとのクラス
（`LightingPass` / `SkyPass` / `FogPass` など。`GetFog()` のようなアクセサで取得）に
分割済みです（CHANGELOG 参照）。パスのインターフェースは新しく、まだ見直す可能性があるため、
レンダラーを直接触っているコードは追従が必要になります。
