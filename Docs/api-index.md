...
エンジンのヘッダを直接読む前に、まずここで型の在処を引く。
シグネチャが要るときだけ `api/<モジュール>.md` を1本だけ開く（全部は開かない）。
...
## Tsukino.Engine — [`api/Tsukino.Engine.md`](api/Tsukino.Engine.md)
...
## Tsukino.Sandbox — [`api/Tsukino.Sandbox.md`](api/Tsukino.Sandbox.md)
