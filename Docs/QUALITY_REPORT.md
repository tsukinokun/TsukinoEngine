...
| C-11 | Sandbox のシーン | 4 シーン中 3 つが PrefabFactory へ移行済み。残るは `DeferredLightSampleScene` のみ。壊れていたアセット参照は解消 |
...
**現状**
4 シーン中 3 つ（`SceneSample1` / `JumpGameSampleScene` / `WaterGameSampleScene`）は
`PrefabFactory` を使うようになっている。
残るは `DeferredLightSampleScene`（316 行）だけで、エンティティ構築がハードコードのまま。

2026-09-10 に `BlockBreakinigSampleScene`（441 行）をブロック崩しサンプルごと削除したため、
ハードコードのシーンは 2 つから 1 つへ減った。削除の際、専用アセットだった
`Assets/Models/Paddle.fbx` と `wall.fbx` も併せて消している
（`Ball.fbx` と `Block.fbx` は `DeferredLightSampleScene` が使うため残置）。

C-2 で `AssetRef` 経由のアセット参照が完成したため、残り 1 つの移行も現実的になっている。
