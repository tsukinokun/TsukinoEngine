...
namespace Tsukino::ECS {
    ...
    public:
        ...
        //--------------------------------------------------------------------
        //! @brief  全エンティティ・全コンポーネントを破棄する
        //! @details
        //! シーンの終了処理から呼ぶ。破棄シグナル（OnDestroy）は通常どおり発火するため、
        //! System が生きている間にこれを呼べば、物理ボディやエフェクトハンドルのような
        //! 「所有権が ECS の外にあるもの」を正規の経路で回収できる。
        //!
        //! Registry のデストラクタ任せにしてはならない。Scene のメンバ破棄順は
        //! System → Registry であり、その時点では OnDestroy の購読者が
        //! 全員 disconnect 済みで、回収処理がどこにも届かないまま
        //! 全エンティティが消えることになる。
        //--------------------------------------------------------------------
        void Clear() {
            // 予約だけ残っていても破棄先が消えるので、先に捨てる
            m_destroyQueue.clear();
            registry.clear();
        }
